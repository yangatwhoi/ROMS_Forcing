...
for T_NUM = T_beg:T_end
    ...
    if IF_Monthly_output==1

    if T_NUM~=T_beg&&...
       T_NUM==Freq*datenum(year,month,1,0,0,0)
        TIME = T_NUM/Freq-1;
        [year_num,month_num,day_num,...
        hour_num,minu_num,seco_num]    = date2str(TIME);
        bryname = [Save_dir,'/',S_file,'_',year_num,month_num,'.nc'];
        grdname = G_file;
        delete(bryname)
        time(find(time<0)) = 0;
    ...
    if T_NUM==T_end&&...
       T_NUM~=Freq*datenum(year,month,1,0,0,0)
        ...
        [year_num,month_num,day_num,...
        hour_num,minu_num,seco_num]    = date2str(TIME);
        bryname = [Save_dir,'/',S_file,'_',year_num,month_num,day_num,'.nc'];
        grdname = G_file;
        delete(bryname)
        time(find(time<0)) = 0;
        create_bryfile(bryname,grdname,title,obc,...
    ...
    elseif IF_Monthly_output==0

    if T_NUM~=T_beg...
     &&T_NUM~=T_end...
     &&(T_NUM-Freq*datenum(year,1,1,0,0,0))==0
